...
def run():
	ngramList =[]
	articles = Articles.objects.filter(Type='Training')[25:26]
	sentence_stream = []
...
def textClean(rawText,ngramList):
	...
	newString = re.sub(r'([a-z]\.[a-z]\.){1,}',toUpperRemovePeriod,newString)
	newString = re.sub(r'[a-z]\.[a-z]\.[a-z]\.',toUpperRemovePeriod,newString)
	# ---------*****************&&&&&&& Above not working for threee letters - Check &&&&&&&&&&&&&
